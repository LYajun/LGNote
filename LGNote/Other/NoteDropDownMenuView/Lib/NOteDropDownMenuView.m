...
@interface NOteDropDownMenuView ()
...
@implementation NOteDropDownMenuView
...
- (UIView *)titleBar
{
    if (!_titleBar) {
        ...
        [_titleBar addGestureRecognizer:tapGest];
...
- (void)setDataSourceArr:(NSMutableArray *)dataSourceArr
{
    ...
    for (NSInteger i=0; i<dataSourceArr.count; i++) {
        ...
        NoteLabelOnLeftButton *titleBtn = nil;
//        CF_Color_TextLightGrayColor
        titleBtn = [NoteLabelOnLeftButton createButtonWithImageNamenote:CFDrowMenuViewSrcName(@"灰箭头.png")?:CFDrowMenuViewFrameworkSrcName(@"灰箭头.png") title:@"" titleColor:CF_Color_TextLightGrayColor frame:CGRectMake(i*btnW, 0, btnW, btnH) target:self action:@selector(titleBtnClicked:)];
...
- (void)setDefaulTitleArray:(NSArray *)defaulTitleArray
{
    ...
    for (NSInteger i = 0; i < self.titleBtnArr.count; i++) {
       ...
       // self.titleBtnArr[i].contentHorizontalAlignment = UIControlContentHorizontalAlignmentLeft;
        

       // [self.titleBtnArr[i] ]
        
//        self.titleBtnArr[i].titleEdgeInsets = UIEdgeInsetsMake(0, 10, 0, 0);
...
- (void)titleBtnClicked:(UIButton *)btn
{
    ...
    !self.chooseBtnBlock?:self.chooseBtnBlock(@"");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    lineView.frame = CGRectMake(18, 43, CFScreenWidth-36, 0.7);
        ...
        if (!btn.enabled) {
            ...
            if (self.stateConfigDict[@"selected"]) {
                ...
                if (image) {  // 使用自己app中的图片
                    [self changTintColorWithTintColor:self.stateConfigDict[@"normal"][0] tintColorImgName:self.stateConfigDict[@"selected"][1] ForButton:self.titleBtnArr[i]];
                } else {  // 使用CFDropDownMenuView.bundle自带的
                ...
                [self changTintColorWithTintColor:CF_Color_TextGrayColor tintColorImgName:CFDrowMenuViewSrcName(@"灰箭头.png")?:CFDrowMenuViewFrameworkSrcName(@"灰箭头.png") ForButton:btn];
            }
        }
